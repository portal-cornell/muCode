...
from eval_rm import evaluate_pref as evaluate
...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    if accelerator.is_main_process:
        if not os.path.isdir(f'{args.output_dir}/evals/'):
            os.makedirs(f'{args.output_dir}/evals/')
    ...
    dataset = DatasetDict({f"{args.dataset_eval_splits}": Dataset.load_from_disk(f'{args.dataset_eval_splits}'),
                           f"{args.dataset_train_splits}": Dataset.load_from_disk(f'{args.dataset_train_splits}')})
    ...
    with accelerator.main_process_first():
        if args.code_parsing:
            def process_example(example):
                ...
                return example
            dataset[args.dataset_train_splits] = dataset[args.dataset_train_splits].map(process_example)
            if args.dataset_eval_splits != args.dataset_train_splits:
                dataset[args.dataset_eval_splits] = dataset[args.dataset_eval_splits].map(process_example)
    ...
    for epoch_id in range(args.num_train_epochs):
        for data in dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                norm_loss = args.l2_coeff * (predicted_reward**2).mean()
                # norm_loss = 0.001 * (torch.clamp(predicted_reward**2, 0, 1)).mean()
                accelerator.backward(loss + norm_loss)
                optimizer.step()
                optimizer.zero_grad()
            ...
            if args.num_evals > 0 and training_step > 1 and training_step % args.eval_freq == 0:
                eval_metrics, table = evaluate(model, eval_dataloader, tokenizer, num_samples=10)
                for key in table:
                    table[key] = gather_object(table[key])
                df = pd.DataFrame(table)

                df.to_csv(f'{args.output_dir}/evals/epoch{epoch_id}.csv')
                if accelerator.is_main_process:
    ...
    '''
    FIXME(arj): There are some issue with saving, fix them !!!
    '''
    if model_config.use_peft:
