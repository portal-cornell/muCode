...
# Arguments
RESULTS_DIR=$2
RESULTS_DIR=${RESULTS_DIR}/rft/${BASE_MODEL_NAME}/
...
PREV_MODEL_PATH=${RESULTS_DIR}/${BASE_MODEL_NAME}
...
convert_data src.preprocessing.convert_to_sft ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME} \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_intermediate
convert_data src.preprocessing.convert_to_sft ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME} \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_intermediate
convert_data src.preprocessing.convert_multi_to_many_script ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_intermediate \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft \
    "${multi_to_many_additional_flags}"
convert_data src.preprocessing.convert_multi_to_many_script ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_intermediate \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft \
    "${multi_to_many_additional_flags}"

SFT_EXP_NAME=${SFT_DATASET}_iter${SFT_ITER}
run_sft $SFT_DATASET ${BASE_MODEL_NAME} $SFT_EXP_NAME train_${ROLLOUT_NAME}_sft test_${ROLLOUT_NAME}_sft ${SFT_TRAIN_EPOCHS} ${SFT_LR} $SFT_PER_DEVICE_TRAIN_BATCH_SIZE ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/ ${NUM_GPUS} ${DIST_PORT}
...
run_viz ${SFT_MODEL_PATH}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME} \
    ${RESULTS_PATH}/SFT/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}/
run_viz ${SFT_MODEL_PATH}/data/${SFT_DATASET}/test_${ROLLOUT_NAME} \
    ${RESULTS_PATH}/SFT/${SFT_DATASET}/test_${ROLLOUT_NAME}/
for eval_dataset in ${EVAL_DATASETS[@]}; do
    run_viz ${SFT_MODEL_PATH}/data/${eval_dataset}/test_${ROLLOUT_NAME} \
        ${RESULTS_PATH}/SFT/${eval_dataset}/test_${ROLLOUT_NAME}/
done
...
MERGE_ROLLOUT_NAME=rollout_iter0
SFT_ITER=2

### Filter out positives
multi_to_many_additional_flags="--relabel_only"
convert_data src.preprocessing.convert_to_sft ${SFT_MODEL_PATH}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME} \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_intermediate
convert_data src.preprocessing.convert_to_sft ${SFT_MODEL_PATH}/data/${SFT_DATASET}/test_${ROLLOUT_NAME} \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_intermediate
convert_data src.preprocessing.convert_multi_to_many_script ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_intermediate \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_new \
    "${multi_to_many_additional_flags}"
convert_data src.preprocessing.convert_multi_to_many_script ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_intermediate \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_new \
    "${multi_to_many_additional_flags}"
convert_data src.preprocessing.merge_datasets ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${MERGE_ROLLOUT_NAME}_sft \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft \
    "--new_dataset_path ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}_sft_new"
convert_data src.preprocessing.merge_datasets ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${MERGE_ROLLOUT_NAME}_sft \
    ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft \
    "--new_dataset_path ${RESULTS_DIR}/${BASE_MODEL_NAME}/data/${SFT_DATASET}/test_${ROLLOUT_NAME}_sft_new"
...
run_viz ${SFT_MODEL_PATH}/rm_${RM_EXP_NAME}/${SFT_DATASET}/${TRAIN_SPLIT}_${ROLLOUT_NAME}/data.csv \
    ${RESULTS_PATH}/SFT/${SFT_DATASET}/rm_${RM_EXP_NAME}/${SFT_DATASET}/${TRAIN_SPLIT}/
run_viz ${SFT_MODEL_PATH}/rm_${RM_EXP_NAME}/${SFT_DATASET}/test_${ROLLOUT_NAME}/data.csv \
    ${RESULTS_PATH}/SFT/${SFT_DATASET}/rm_${RM_EXP_NAME}/${SFT_DATASET}/test/
for eval_dataset in ${EVAL_DATASETS[@]}; do
    run_viz ${SFT_MODEL_PATH}/rm_${RM_EXP_NAME}/${eval_dataset}/test_${ROLLOUT_NAME}/data.csv \
        ${RESULTS_PATH}/SFT/${SFT_DATASET}/rm_${RM_EXP_NAME}/${eval_dataset}/test/
done
...
for i in {1..2}; do
  ...
  cp -r ${RESULTS_PREFIX}/${TRAIN_SPLIT}_rollout_iter${i}/* "${SUMMARY_PATH}/iter${i}_${CLEANED_SFT_DATASET}_${TRAIN_SPLIT}/"
  mkdir -p "${SUMMARY_PATH}/iter${i}_${CLEANED_SFT_DATASET}_test/"
  cp -r ${RESULTS_PREFIX}/test_rollout_iter${i}/* "${SUMMARY_PATH}/iter${i}_${CLEANED_SFT_DATASET}_test/"
  for eval_dataset in ${EVAL_DATASETS[@]}; do
    CLEANED_EVAL_DATASET=${eval_dataset//\//_}
    mkdir -p "${SUMMARY_PATH}/iter${i}_${CLEANED_EVAL_DATASET}_test/"
    cp -r ${RESULTS_PREFIX}/test_rollout_iter${i}/* "${SUMMARY_PATH}/iter${i}_${CLEANED_EVAL_DATASET}_test/"
  done
done
mkdir -p "${SUMMARY_PATH}/rm_iter2_${CLEANED_SFT_DATASET}_${TRAIN_SPLIT}/"
cp -r ${RESULTS_PREFIX}/rm_${SFT_DATASET}_iter2/${SFT_DATASET}/${TRAIN_SPLIT}/* "${SUMMARY_PATH}/rm_iter2_${CLEANED_SFT_DATASET}_${TRAIN_SPLIT}/"
mkdir -p "${SUMMARY_PATH}/rm_iter2_${CLEANED_SFT_DATASET}_test/"
cp -r ${RESULTS_PREFIX}/rm_${SFT_DATASET}_iter2/${SFT_DATASET}/test/* "${SUMMARY_PATH}/rm_iter2_${CLEANED_SFT_DATASET}_test/"
for eval_dataset in ${EVAL_DATASETS[@]}; do
    CLEANED_EVAL_DATASET=${eval_dataset//\//_}
    mkdir -p "${SUMMARY_PATH}/rm_iter2_${CLEANED_EVAL_DATASET}_test/"
    cp -r ${RESULTS_PREFIX}/rm_${SFT_DATASET}_iter2/${eval_dataset}/test/* "${SUMMARY_PATH}/rm_iter2_${CLEANED_EVAL_DATASET}_test/"
done